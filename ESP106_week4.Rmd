...
# 2a. Use a for-loop (INSTEAD WE ARE USING APPLY FUNCTION) to read in the csv files and bind them together into a single data frame. Add a column to the data frame giving the name of the city the data is from.
...
```
Hint: you can use list.files() to get a vector of all the file names in a directory. Setting full.names=TRUE in the list.files() function will give you the full path name that can be used by read.csv (check) 

Hint 2: you might want to create a data frame using the first csv file. Then loop through from files 2 to 6 reading in the csv file and using rbind() to attach it to the bottom of your full data frame (check)

Hint 3: It will be easiest to add the names of the city to the data frames immediately after reading them in, before binding everything together into a single data-frame (check)
...
3b. Use as.Date (USED lubridate::make_date) to convert your new date column to a date object in R
...
This coefficient gives us the average increase in high tide levels each day (OR IS IT MONTH?), across all six cities, for this ten year time frame (i.e. the units of the coefficient are in m per day).
